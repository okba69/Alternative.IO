...
export function RequestsBoard() {
  ...
  return <>
    <section className="request-board-head"><div><p className="eyebrow">Demandes ouvertes</p><h2>Les questions auxquelles le catalogue ne répond pas encore.</h2><p>Une demande concerne un outil payant précis. La communauté peut commenter et proposer une alternative.</p></div><GoogleAuthButton /></section>
    <div className="request-layout">
      ...
      <aside className="request-side"><SessionPanel /><button className="button button-primary request-action" type="button" onClick={() => setShowContribution((current) => !current)}>{showContribution ? 'Fermer le formulaire' : 'Proposer une alternative'} <span>＋</span></button><p className="request-side-note">Deux parcours distincts : propose une fiche directement, ou crée une demande pour qu’un autre membre trouve une alternative.</p></aside>
    </div>
    ...
    {showContribution && <section className="contribution-section"><p className="eyebrow">Proposition d’alternative</p><h2>Ajoute une alternative au catalogue.</h2><p className="inner-lede">Tu connais déjà l’outil payant et son alternative ? Renseigne-les ici pour publier une fiche directement.</p><ContributionForm /></section>}
  </>;
}
