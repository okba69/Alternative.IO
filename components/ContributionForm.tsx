...
export function ContributionForm() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const paidProducts = products.map((product) => ({ name: product.name.trim(), url: normalizeUrl(product.url), description: product.description.trim(), price: formatPrice(product), amount: Number(product.amount), currency: product.currency, billing: product.billing }));
      const payload = { paid_name: first.name.trim(), paid_url: normalizeUrl(first.url), paid_description: first.description.trim(), paid_price: formatPrice(first), paid_price_amount: Number(first.amount), paid_currency: first.currency, paid_billing_period: first.billing, paid_products: paidProducts, ...form, alternative_url: normalizeUrl(form.alternative_url), tags: form.tags.split(',').map((tag) => tag.trim()).filter(Boolean), limits: [] as string[], platforms: [] as string[], created_by: user.id };
      let { error } = await supabase.from('alternative_pairs').insert(payload as never);
      if (error && (error.code === '42703' || error.message.includes('paid_products') || error.message.includes('paid_price_amount'))) { const legacyPayload = { paid_name: first.name.trim(), paid_url: normalizeUrl(first.url), paid_description: first.description.trim(), paid_price: formatPrice(first), alternative_name: form.alternative_name.trim(), alternative_url: normalizeUrl(form.alternative_url), alternative_description: form.alternative_description.trim(), alternative_type: form.alternative_type, category: form.category, tags: form.tags.split(',').map((tag) => tag.trim()).filter(Boolean), limits: [] as string[], platforms: [] as string[], created_by: user.id }; ({ error } = await supabase.from('alternative_pairs').insert(legacyPayload as never)); }
