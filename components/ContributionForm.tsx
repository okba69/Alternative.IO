...
function formatPrice(product: PaidProduct) {
  const symbol = product.currency === 'EUR' ? '€' : product.currency === 'GBP' ? '£' : '$';
  return `${product.amount} ${symbol}${product.billing === 'annual' ? '/an' : '/mois'}`;
}
...
export function ContributionForm() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (error && (error.code === '42703' || error.message.includes('paid_products') || error.message.includes('paid_price_amount'))) {
        const legacyPayload = { paid_name: first.name.trim(), paid_url: first.url.trim(), paid_description: first.description.trim(), paid_price: formatPrice(first), alternative_name: form.alternative_name.trim(), alternative_url: form.alternative_url.trim(), alternative_description: form.alternative_description.trim(), alternative_type: form.alternative_type, category: form.category, tags: form.tags.split(',').map((tag) => tag.trim()).filter(Boolean), limits: [] as string[], platforms: [] as string[], created_by: user.id };
        ({ error } = await supabase.from('alternative_pairs').insert(legacyPayload as never));
      }
