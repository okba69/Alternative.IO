...
type AdminProposal = { id: string; request_id: string; alternative_name: string; explanation: string; status: 'pending' | 'approved' | 'rejected'; created_at: string };
...
export function AdminPage() {
  ...
  async function loadAdmin() {
    try {
      ...
      setProfiles((profilesResult.data ?? []) as AdminProfile[]); setPairs((pairsResult.data ?? []) as AdminPair[]); setRequests((requestsResult.data ?? []) as AdminRequest[]); setProposals((proposalsResult.data ?? []) as AdminProposal[]); setState('ready');
  ...
  return <main className="admin-page"><header className="admin-header"><div><Link className="account-back" href="/">← Catalogue</Link><p className="account-kicker">USEINSTEAD / ADMIN</p><h1>Pilotage du catalogue.</h1><p>Gestion réservée aux profils admin vérifiés par Supabase.</p></div><Link className="account-link" href="/account">Mon espace utilisateur ↗</Link></header><div className="account-stats"><div><strong>{pairs.filter((item) => item.status === 'pending').length + requests.filter((item) => item.status === 'pending').length + proposals.filter((item) => item.status === 'pending').length}</strong><span>À valider</span></div><div><strong>{pairs.filter((item) => item.status === 'approved').length + proposals.filter((item) => item.status === 'approved').length}</strong><span>Approuvées</span></div><div><strong>{profiles.length}</strong><span>Profils</span></div></div>{message && <p className="form-success dark-error">{message}</p>}<section className="admin-section"><div className="account-section-heading"><div><p className="account-kicker">01 / Modération</p><h2>Alternatives proposées</h2></div></div><div className="admin-table">{pairs.map((item) => <div className="admin-row" key={item.id}><span><strong>{item.paid_name} → {item.alternative_name}</strong><small>{item.category} · {item.status === 'pending' ? 'En attente' : item.status === 'approved' ? 'Approuvée' : 'Refusée'}</small></span>{item.status !== 'approved' && <button type="button" onClick={() => void updatePairStatus(item.id, 'approved')}>Approuver</button>}{item.status !== 'rejected' && <button type="button" onClick={() => void updatePairStatus(item.id, 'rejected')}>Refuser</button>}<Link href={`/catalogue/${item.id}`}>Voir</Link><button type="button" onClick={() => void deletePair(item.id)}>Supprimer</button></div>)}{pairs.length === 0 && <p className="account-empty-line">Aucune proposition à modérer.</p>}</div></section><section className="admin-section"><div className="account-section-heading"><div><p className="account-kicker">02 / Requests</p><h2>Requests à valider</h2></div></div><div className="admin-table">{requests.map((item) => <div className="admin-row" key={item.id}><span><strong>{item.title}</strong><small>{item.category} · {item.status === 'pending' ? 'En attente' : item.status === 'approved' ? 'Approuvée' : 'Refusée'}</small></span>{item.status !== 'approved' && <button type="button" onClick={() => void updateRequestStatus(item.id, 'approved')}>Approuver</button>}{item.status !== 'rejected' && <button type="button" onClick={() => void updateRequestStatus(item.id, 'rejected')}>Refuser</button>}<Link href={`/requests/${item.id}`}>Voir</Link><button type="button" onClick={() => void deleteRequest(item.id)}>Supprimer</button></div>)}{requests.length === 0 && <p className="account-empty-line">Aucune request persistée.</p>}</div></section><section className="admin-section"><div className="account-section-heading"><div><p className="account-kicker">03 / Réponses aux requests</p><h2>Réponses à valider</h2></div></div><div className="admin-table">{proposals.map((item) => <div className="admin-row" key={item.id}><span><strong>{item.alternative_name}</strong><small>{item.explanation} · {item.status === 'pending' ? 'En attente' : item.status === 'approved' ? 'Approuvée' : 'Refusée'}</small></span>{item.status !== 'approved' && <button type="button" onClick={() => void updateProposalStatus(item.id, 'approved')}>Approuver</button>}{item.status !== 'rejected' && <button type="button" onClick={() => void updateProposalStatus(item.id, 'rejected')}>Refuser</button>}<Link href={`/requests/${item.request_id}`}>Voir</Link><button type="button" onClick={() => void deleteProposal(item.id)}>Supprimer</button></div>)}{proposals.length === 0 && <p className="account-empty-line">Aucune réponse à modérer.</p>}</div></section><section className="admin-section"><div className="account-section-heading"><div><p className="account-kicker">04 / Profils</p><h2>Utilisateurs</h2></div></div><div className="admin-table">{profiles.map((item) => <div className="admin-row" key={item.id}><span><strong>{item.display_name || 'Profil sans nom'}</strong><small>{new Date(item.created_at).toLocaleDateString('fr-FR')}</small></span><b>{item.role}</b></div>)}</div></section></main>;
}
