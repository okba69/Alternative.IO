...
export function RequestDetail({ requestId }: RequestDetailProps) {
  ...
  async function submitProposal(event: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.from('request_proposals').insert({ ...proposal, alternative_url: normalizeUrl(proposal.alternative_url), request_id: requestId, created_by: data.user.id, status: 'pending' } as never);
